...
using namespace SMR;
namespace VQTL {
    void load_vqtl_workspace(eInfo* einfo,bInfo* bdata, char* efileName, char* befileName, char* phenofileName, char* bFileName, bool transposed, int efileType,char* problstName,char* problst2exclde,char* genelistName, int chr,char* prbname, char* fromprbname, char* toprbname,int prbWind,int fromprbkb, int toprbkb,bool prbwindFlag, char* genename,char* probe2exclde,char* indilstName,char* indilst2remove, bool no_fid_flag,int valueType,bool beta2m,bool m2beta, double std_thresh,double upperBeta,double lowerBeta,char* dpvalfName, double dp_thresh, double prb_thresh, double spl_thresh, int filter_mth, double mssratio_prob, int autosome_num,char* snplstName,char* snplst2exclde,int tsk_ttl,int tsk_id, char* covfileName, char* qcovfileName,char* grm_file=NULL,int xqtlNO = 0, double zeroratio = 0.2, eInfo* eCov = NULL, char* covbodfileName = NULL, char* covefileName = NULL, bool transopse_ecov = false);
    ...
    void sQTL(char* outFileName,  char* efileName, char* befileName, \
        ...
        double prb_thresh, double spl_thresh, int filter_mth, \
        double mssratio_prob, int autosome_num, double maf, char* snplstName, \
        char* snplst2exclde, int tsk_ttl, int tsk_id, char* covfileName, \
        char* qcovfileName, bool tosmrflag, bool nofastlinear, bool cis_flag, \
        int cis_itvl, double zeroratio, double call, char* annofileName, \
        char* covbodfileName, char* covefileName, bool transopse_ecov);
